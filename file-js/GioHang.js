...
function cartIotal(){
    ...
    var TotalA = 0
    for( i=0; i < cartItem.length; i++){
        var inputValue = cartItem[i].querySelectorAll("#input").value
        var productPrice = cartItem[i].querySelectorAll("#price").innerHTML
        TotalB = inputValue*productPrice*1000
        TotalA = TotalA + TotalB
    }

    var tst = document.querySelector("#TST #span1")
    tst.innerHTML = TotalA.toLocaleString()
    console.log(tst)
